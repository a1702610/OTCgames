  ...
  return (
    <div style={{ padding: 16, background: 'rgba(255,255,255,0.78)', border: '1px solid rgba(131,107,255,0.20)', borderRadius: 12, backdropFilter: 'blur(20px)' }}>
      ...
      {question.categories.map((cat) => (
        <div key={cat.id} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
          <input
            ...
            style={{ width: 32, height: 32, border: '1px solid rgba(255,255,255,0.10)', borderRadius: 6, cursor: 'pointer', background: 'transparent' }}
      ...
      {question.categories.length < 4 && (
        <button
          onClick={addCategory}
          style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#7da5ff', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 12 }}
        >
            ...
            <div key={product.id} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              {isIncluded && (
                <select
                  ...
                  style={{ fontSize: 12, padding: '3px 8px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.18)', background: '#1a1560', color: '#FFFFFF', cursor: 'pointer' }}
